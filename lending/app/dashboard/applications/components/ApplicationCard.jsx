...
export default function ApplicationCard({ application }) {
  ...
  const lenderName = application.lender?.profile?.name || "Anonymous Lender";
  ...
  return (
    <div className="bg-white rounded-2xl border border-[rgba(245,166,35,0.12)] shadow-sm p-6 hover:shadow-md transition-all duration-200 flex flex-col justify-between h-full">
      <div>
        ...
        <h3 className="text-2xl font-extrabold text-[#16120A] mb-1">
          {amount}
        </h3>
        <p className="text-xs text-[#6B5E45] mb-4">
          Lender: <strong className="text-[#16120A]">{lenderName}</strong>
        </p>
      </div>
